...
| ---------------------- | :---------------- | --: | -----: | ----: | ------: | ------: | ------: | -------: |
| `bucket_logging`       | **0%** (0/6)      |   0 |      6 |     6 |  7.403s |  0.708s |  6.567s |   8.519s |
| `bucket_policy`        | **17%** (2/12)    |   2 |     10 |    12 |  7.695s |  2.428s |  5.467s |  15.156s |
| `checksum`             | **91%** (10/11)   |  10 |      1 |    11 | 48.697s | 43.862s | 12.806s | 179.491s |
| `conditional_write`    | **43%** (3/7)     |   3 |      4 |     7 | 10.442s |  3.282s |  6.364s |  15.990s |
| `copy`                 | **68%** (21/31)   |  21 |     10 |    31 | 30.432s | 52.205s |  5.407s | 205.310s |
| `delete_marker`        | **100%** (3/3)    |   3 |      0 |     3 |  7.462s |  0.682s |  6.627s |   8.297s |
| `encryption`           | **56%** (61/108)  |  61 |     47 |   108 | 43.982s | 82.010s |  5.353s | 341.688s |
| `lifecycle`            | **80%** (12/15)   |  12 |      3 |    15 |  5.659s |  0.296s |  5.230s |   6.400s |
| `lifecycle_expiration` | **67%** (4/6)     |   4 |      2 |     6 | 16.700s | 23.010s |  6.058s |  68.144s |
| `list_objects_v2`      | **85%** (40/47)   |  40 |      7 |    47 |  7.286s |  1.767s |  4.141s |  16.247s |
| `s3_core`              | **65%** (209/324) | 209 |    115 |   324 | 19.127s | 64.104s |  4.062s | 798.470s |
| `sse_s3`               | **33%** (1/3)     |   1 |      2 |     3 |  5.816s |  0.386s |  5.297s |   6.221s |
| `tagging`              | **8%** (2/25)     |   2 |     23 |    25 | 15.421s |  8.480s |  5.425s |  27.775s |
...
TEST RUN
  ...
  Raw JSON  : ../logs/20260706_115232_compatibility_pytest_main.json, ../logs/20260706_115232_compatibility_pytest_quarantine.json
```
...
**Error stack trace** — `list_objects_v2` — `failed` — 16.247s
...
**Error stack trace** — `list_objects_v2` — `failed` — 5.122s
...
**Error stack trace** — `s3_core` — `failed` — 4.965s
...
**Error stack trace** — `s3_core` — `failed` — 5.121s
...
**Error stack trace** — `list_objects_v2` — `failed` — 5.211s
...
**Error stack trace** — `s3_core` — `failed` — 5.08s
...
**Error stack trace** — `list_objects_v2` — `failed` — 5.316s
...
**Error stack trace** — `s3_core` — `failed` — 429.929s
...
**Error stack trace** — `s3_core` — `failed` — 7.237s
...
**Error stack trace** — `s3_core` — `failed` — 8.525s
...
**Error stack trace** — `s3_core` — `failed` — 20.502s
...
**Error stack trace** — `s3_core` — `failed` — 5.556s
...
**Error stack trace** — `s3_core` — `failed` — 6.575s
...
**Error stack trace** — `s3_core` — `failed` — 6.564s
...
**Error stack trace** — `s3_core` — `failed` — 6.152s
...
**Error stack trace** — `s3_core` — `failed` — 6.053s
...
**Error stack trace** — `s3_core` — `failed` — 7.651s
...
**Error stack trace** — `s3_core` — `failed` — 11.531s
...
**Error stack trace** — `s3_core` — `failed` — 12.094s
...
**Error stack trace** — `s3_core` — `failed` — 16.163s
...
**Error stack trace** — `s3_core` — `failed` — 12.884s
...
**Error stack trace** — `s3_core` — `failed` — 19.05s
...
**Error stack trace** — `s3_core` — `failed` — 16.763s
...
**Error stack trace** — `s3_core` — `failed` — 9.749s
...
**Error stack trace** — `s3_core` — `failed` — 12.406s
...
**Error stack trace** — `s3_core` — `failed` — 12.698s
...
**Error stack trace** — `s3_core` — `failed` — 15.283s
...
**Error stack trace** — `s3_core` — `failed` — 14.226s
...
**Error stack trace** — `s3_core` — `failed` — 17.951s
...
**Error stack trace** — `s3_core` — `failed` — 18.734s
...
**Error stack trace** — `s3_core` — `failed` — 18.781s
...
**Error stack trace** — `s3_core` — `failed` — 18.03s
...
**Error stack trace** — `s3_core` — `failed` — 8.634s
...
**Error stack trace** — `s3_core` — `failed` — 11.175s
...
**Error stack trace** — `s3_core` — `failed` — 13.84s
...
**Error stack trace** — `s3_core` — `failed` — 16.179s
...
**Error stack trace** — `s3_core` — `failed` — 9.42s
...
**Error stack trace** — `s3_core` — `failed` — 17.935s
...
**Error stack trace** — `s3_core` — `failed` — 18.219s
...
**Error stack trace** — `s3_core` — `failed` — 5.248s
...
**Error stack trace** — `s3_core` — `failed` — 5.381s
...
**Error stack trace** — `s3_core` — `failed` — 5.283s
...
**Error stack trace** — `s3_core` — `failed` — 5.082s
...
**Error stack trace** — `s3_core` — `failed` — 5.194s
...
**Error stack trace** — `s3_core` — `failed` — 5.434s
...
**Error stack trace** — `s3_core` — `failed` — 6.284s
...
**Error stack trace** — `s3_core` — `failed` — 6.398s
...
**Error stack trace** — `s3_core` — `failed` — 6.776s
...
**Error stack trace** — `s3_core` — `failed` — 6.19s
...
**Error stack trace** — `s3_core` — `failed` — 6.061s
...
**Error stack trace** — `s3_core` — `failed` — 6.346s
...
**Error stack trace** — `s3_core` — `failed` — 6.198s
...
**Error stack trace** — `s3_core` — `failed` — 6.431s
...
**Error stack trace** — `s3_core` — `failed` — 5.491s
...
**Error stack trace** — `s3_core` — `failed` — 5.29s
...
**Error stack trace** — `s3_core` — `failed` — 6.037s
...
**Error stack trace** — `s3_core` — `failed` — 6.156s
...
**Error stack trace** — `list_objects_v2` — `failed` — 6.112s
...
**Error stack trace** — `s3_core` — `failed` — 6.121s
...
**Error stack trace** — `list_objects_v2` — `failed` — 6.147s
...
**Error stack trace** — `s3_core` — `failed` — 6.248s
...
**Error stack trace** — `list_objects_v2` — `failed` — 6.145s
...
**Error stack trace** — `s3_core` — `failed` — 6.224s
...
**Error stack trace** — `s3_core` — `failed` — 6.266s
...
**Error stack trace** — `s3_core` — `failed` — 6.476s
...
**Error stack trace** — `s3_core` — `failed` — 6.414s
...
**Error stack trace** — `s3_core` — `failed` — 6.251s
...
**Error stack trace** — `s3_core` — `failed` — 6.294s
...
**Error stack trace** — `s3_core` — `failed` — 4.071s

```pytb
s3tests/functional/test_s3.py:5407: in test_list_buckets_paginated
    assert len(response['Buckets']) == 0
E   AssertionError: assert 1 == 0
E    +  where 1 = len([{'CreationDate': datetime.datetime(2026, 7, 3, 5, 29, 31, tzinfo=tzutc()), 'Name': 'probe-1783056572747'}])
```
...
**Error stack trace** — `s3_core` — `failed` — 5.062s
...
**Error stack trace** — `copy` — `failed` — 17.664s
...
**Error stack trace** — `copy` — `failed` — 205.31s
...
**Error stack trace** — `copy` — `failed` — 7.57s
...
**Error stack trace** — `copy` — `failed` — 6.142s
...
**Error stack trace** — `copy` — `failed` — 5.701s
...
**Error stack trace** — `copy` — `failed` — 104.046s
...
**Error stack trace** — `s3_core` — `failed` — 37.433s
...
**Error stack trace** — `copy` — `failed` — 71.856s
...
**Error stack trace** — `s3_core` — `failed` — 34.656s
...
**Error stack trace** — `s3_core` — `failed` — 16.478s
...
**Error stack trace** — `s3_core` — `failed` — 51.749s
...
**Error stack trace** — `s3_core` — `failed` — 6.846s
...
**Error stack trace** — `s3_core` — `failed` — 5.388s
...
**Error stack trace** — `s3_core` — `failed` — 5.51s
...
**Error stack trace** — `s3_core` — `failed` — 5.339s
...
**Error stack trace** — `s3_core` — `failed` — 5.446s
...
**Error stack trace** — `s3_core` — `failed` — 16.913s
...
**Error stack trace** — `s3_core` — `failed` — 16.068s
...
**Error stack trace** — `s3_core` — `failed` — 4.137s
...
**Error stack trace** — `s3_core` — `failed` — 4.062s
...
**Error stack trace** — `s3_core` — `failed` — 15.666s
...
**Error stack trace** — `s3_core` — `failed` — 9.698s
...
**Error stack trace** — `s3_core` — `failed` — 4.076s
...
**Error stack trace** — `s3_core` — `failed` — 4.082s
...
**Error stack trace** — `s3_core` — `failed` — 13.419s
...
**Error stack trace** — `s3_core` — `failed` — 12.733s
...
**Error stack trace** — `tagging` — `failed` — 5.425s
...
**Error stack trace** — `s3_core` — `failed` — 16.576s
...
**Error stack trace** — `s3_core` — `failed` — 11.589s
...
**Error stack trace** — `s3_core` — `failed` — 15.596s
...
**Error stack trace** — `s3_core` — `failed` — 6.777s
...
**Error stack trace** — `s3_core` — `failed` — 14.012s
...
**Error stack trace** — `s3_core` — `failed` — 43.083s
...
<details markdown="1">
<summary>s3tests/functional/test_s3.py::test_versioning_obj_create_overwrite_multipart</summary>

**Test description**

> Three sequential multipart uploads to the same key under versioning produce three distinct VersionIds; verifies each version's contents and supports per-version delete.

**Error stack trace** — `s3_core` — `failed` — 243.641s

```pytb
../.venv/lib/python3.14/site-packages/urllib3/response.py:905: in _error_catcher
    yield
../.venv/lib/python3.14/site-packages/urllib3/response.py:1022: in _raw_read
    data = self._fp_read(amt, read1=read1) if not fp_closed else b""
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/urllib3/response.py:1005: in _fp_read
    return self._fp.read(amt) if amt is not None else self._fp.read()
                                                      ^^^^^^^^^^^^^^^
/Users/srdjan/.local/share/uv/python/cpython-3.14.4-macos-aarch64-none/lib/python3.14/http/client.py:500: in read
    s = self._safe_read(self.length)
        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^
/Users/srdjan/.local/share/uv/python/cpython-3.14.4-macos-aarch64-none/lib/python3.14/http/client.py:660: in _safe_read
    data.write(self.fp.read(delta))
               ^^^^^^^^^^^^^^^^^^^
/Users/srdjan/.local/share/uv/python/cpython-3.14.4-macos-aarch64-none/lib/python3.14/socket.py:725: in readinto
    return self._sock.recv_into(b)
           ^^^^^^^^^^^^^^^^^^^^^^^
/Users/srdjan/.local/share/uv/python/cpython-3.14.4-macos-aarch64-none/lib/python3.14/ssl.py:1316: in recv_into
    return self.read(nbytes, buffer)
           ^^^^^^^^^^^^^^^^^^^^^^^^^
/Users/srdjan/.local/share/uv/python/cpython-3.14.4-macos-aarch64-none/lib/python3.14/ssl.py:1150: in read
    return self._sslobj.read(len, buffer)
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
E   TimeoutError: The read operation timed out

The above exception was the direct cause of the following exception:
../.venv/lib/python3.14/site-packages/botocore/response.py:98: in read
    chunk = self._raw_stream.read(amt)
            ^^^^^^^^^^^^^^^^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/urllib3/response.py:1110: in read
    data = self._raw_read(amt)
           ^^^^^^^^^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/urllib3/response.py:1021: in _raw_read
    with self._error_catcher():
         ^^^^^^^^^^^^^^^^^^^^^
/Users/srdjan/.local/share/uv/python/cpython-3.14.4-macos-aarch64-none/lib/python3.14/contextlib.py:162: in __exit__
    self.gen.throw(value)
../.venv/lib/python3.14/site-packages/urllib3/response.py:910: in _error_catcher
    raise ReadTimeoutError(self._pool, None, "Read timed out.") from e  # type: ignore[arg-type]
    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
E   urllib3.exceptions.ReadTimeoutError: AWSHTTPSConnectionPool(host='us-east-1.s3.fil.one', port=443): Read timed out.

During handling of the above exception, another exception occurred:
s3tests/functional/test_s3.py:8053: in test_versioning_obj_create_overwrite_multipart
    check_obj_versions(client, bucket_name, key, version_ids, contents)
s3tests/functional/test_s3.py:7684: in check_obj_versions
    check_obj_content(client, bucket_name, key, version['VersionId'], contents[i])
s3tests/functional/test_s3.py:7666: in check_obj_content
    body = _get_body(response)
           ^^^^^^^^^^^^^^^^^^^
s3tests/functional/test_s3.py:1832: in _get_body
    got = body.read()
          ^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/botocore/response.py:101: in read
    raise ReadTimeoutError(endpoint_url=e.url, error=e)
E   botocore.exceptions.ReadTimeoutError: Read timeout on endpoint URL: "None"
```
...
**Error stack trace** — `s3_core` — `failed` — 8.177s
...
**Error stack trace** — `s3_core` — `failed` — 8.266s
...
**Error stack trace** — `lifecycle` — `failed` — 6.013s
...
**Error stack trace** — `lifecycle` — `failed` — 5.607s
...
**Error stack trace** — `lifecycle` — `failed` — 5.834s
...
**Error stack trace** — `lifecycle_expiration` — `failed` — 6.171s
...
**Error stack trace** — `lifecycle_expiration` — `failed` — 6.433s
...
**Error stack trace** — `encryption` — `failed` — 5.472s
...
**Error stack trace** — `encryption` — `failed` — 5.697s
...
**Error stack trace** — `encryption` — `failed` — 5.353s
...
**Error stack trace** — `encryption` — `failed` — 5.422s
...
**Error stack trace** — `encryption` — `failed` — 5.458s
...
**Error stack trace** — `encryption` — `failed` — 5.454s
...
**Error stack trace** — `encryption` — `failed` — 5.608s
...
**Error stack trace** — `encryption` — `failed` — 5.479s
...
**Error stack trace** — `encryption` — `failed` — 170.466s
...
**Error stack trace** — `encryption` — `failed` — 5.562s
...
**Error stack trace** — `bucket_policy` — `failed` — 6.96s
...
**Error stack trace** — `bucket_policy` — `failed` — 7.013s
...
**Error stack trace** — `bucket_policy` — `failed` — 6.712s
...
**Error stack trace** — `bucket_policy` — `failed` — 5.467s
...
**Error stack trace** — `bucket_policy` — `failed` — 6.348s
...
**Error stack trace** — `bucket_policy` — `failed` — 8.352s
...
**Error stack trace** — `bucket_policy` — `failed` — 8.162s
...
**Error stack trace** — `bucket_policy` — `failed` — 15.156s
...
**Error stack trace** — `bucket_policy` — `failed` — 6.184s
...
**Error stack trace** — `tagging` — `failed` — 6.767s
...
**Error stack trace** — `tagging` — `failed` — 21.387s
...
**Error stack trace** — `tagging` — `failed` — 25.932s
...
**Error stack trace** — `tagging` — `failed` — 22.295s
...
**Error stack trace** — `tagging` — `failed` — 24.463s
...
**Error stack trace** — `tagging` — `failed` — 23.779s
...
**Error stack trace** — `tagging` — `failed` — 24.04s
...
**Error stack trace** — `tagging` — `failed` — 22.009s
...
**Error stack trace** — `tagging` — `failed` — 22.894s
...
**Error stack trace** — `tagging` — `failed` — 27.775s
...
**Error stack trace** — `tagging` — `failed` — 8.37s
...
**Error stack trace** — `tagging` — `failed` — 6.395s
...
**Error stack trace** — `tagging` — `failed` — 24.649s
...
**Error stack trace** — `tagging` — `failed` — 26.336s
...
**Error stack trace** — `tagging` — `failed` — 23.471s
...
**Error stack trace** — `s3_core` — `failed` — 19.139s
...
**Error stack trace** — `tagging` — `failed` — 7.313s
...
**Error stack trace** — `tagging` — `failed` — 7.147s
...
**Error stack trace** — `tagging` — `failed` — 7.279s
...
**Error stack trace** — `copy` — `failed` — 11.792s
...
**Error stack trace** — `tagging` — `failed` — 10.445s
...
**Error stack trace** — `tagging` — `failed` — 10.245s
...
**Error stack trace** — `bucket_policy` — `failed` — 8.732s
...
**Error stack trace** — `encryption` — `failed` — 17.709s
...
**Error stack trace** — `encryption` — `failed` — 16.506s
...
**Error stack trace** — `encryption` — `failed` — 5.548s
...
**Error stack trace** — `tagging` — `failed` — 6.467s
...
**Error stack trace** — `tagging` — `failed` — 7.406s
...
**Error stack trace** — `copy` — `failed` — 6.068s
...
**Error stack trace** — `copy` — `failed` — 5.806s
...
**Error stack trace** — `s3_core` — `failed` — 5.838s
...
**Error stack trace** — `s3_core` — `failed` — 5.613s
...
**Error stack trace** — `s3_core` — `failed` — 5.481s
...
**Error stack trace** — `s3_core` — `failed` — 5.429s
...
**Error stack trace** — `s3_core` — `failed` — 6.095s
...
**Error stack trace** — `s3_core` — `failed` — 5.293s
...
**Error stack trace** — `s3_core` — `failed` — 5.528s
...
**Error stack trace** — `s3_core` — `failed` — 6.013s
...
**Error stack trace** — `s3_core` — `failed` — 5.656s
...
**Error stack trace** — `s3_core` — `failed` — 5.691s
...
**Error stack trace** — `s3_core` — `failed` — 5.482s
...
**Error stack trace** — `s3_core` — `failed` — 19.637s
...
**Error stack trace** — `s3_core` — `failed` — 5.389s
...
**Error stack trace** — `s3_core` — `failed` — 7.811s
...
**Error stack trace** — `s3_core` — `failed` — 6.07s
...
**Error stack trace** — `sse_s3` — `failed` — 5.297s
...
**Error stack trace** — `encryption` — `failed` — 5.378s
...
**Error stack trace** — `sse_s3` — `failed` — 5.929s
...
**Error stack trace** — `encryption` — `failed` — 5.655s
...
**Error stack trace** — `encryption` — `failed` — 5.684s
...
**Error stack trace** — `encryption` — `failed` — 6.064s
...
**Error stack trace** — `encryption` — `failed` — 315.956s

```pytb
../.venv/lib/python3.14/site-packages/urllib3/connectionpool.py:788: in urlopen
    response = self._make_request(
../.venv/lib/python3.14/site-packages/urllib3/connectionpool.py:493: in _make_request
    conn.request(
../.venv/lib/python3.14/site-packages/botocore/awsrequest.py:96: in request
    rval = super().request(method, url, body, headers, *args, **kwargs)
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/urllib3/connection.py:512: in request
    self.send(b"%x\r\n%b\r\n" % (len(chunk), chunk))
../.venv/lib/python3.14/site-packages/botocore/awsrequest.py:223: in send
    return super().send(str)
           ^^^^^^^^^^^^^^^^^
/Users/srdjan/.local/share/uv/python/cpython-3.14.4-macos-aarch64-none/lib/python3.14/http/client.py:1077: in send
    self.sock.sendall(data)
/Users/srdjan/.local/share/uv/python/cpython-3.14.4-macos-aarch64-none/lib/python3.14/ssl.py:1275: in sendall
    v = self.send(byte_view[count:])
        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^
/Users/srdjan/.local/share/uv/python/cpython-3.14.4-macos-aarch64-none/lib/python3.14/ssl.py:1244: in send
    return self._sslobj.write(data)
           ^^^^^^^^^^^^^^^^^^^^^^^^
E   TimeoutError: The write operation timed out

During handling of the above exception, another exception occurred:
../.venv/lib/python3.14/site-packages/botocore/httpsession.py:477: in send
    urllib_response = conn.urlopen(
../.venv/lib/python3.14/site-packages/urllib3/connectionpool.py:842: in urlopen
    retries = retries.increment(
../.venv/lib/python3.14/site-packages/urllib3/util/retry.py:473: in increment
    raise reraise(type(error), error, _stacktrace)
          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/urllib3/util/util.py:38: in reraise
...
/Users/srdjan/.local/share/uv/python/cpython-3.14.4-macos-aarch64-none/lib/python3.14/ssl.py:1244: in send
    return self._sslobj.write(data)
           ^^^^^^^^^^^^^^^^^^^^^^^^
E   urllib3.exceptions.ProtocolError: ('Connection aborted.', TimeoutError('The write operation timed out'))
...
../.venv/lib/python3.14/site-packages/botocore/context.py:123: in wrapper
    return func(*args, **kwargs)
           ^^^^^^^^^^^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/botocore/client.py:1060: in _make_api_call
    http, parsed_response = self._make_request(
../.venv/lib/python3.14/site-packages/botocore/client.py:1084: in _make_request
    return self._endpoint.make_request(operation_model, request_dict)
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/botocore/endpoint.py:119: in make_request
    return self._send_request(request_dict, operation_model)
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/botocore/endpoint.py:200: in _send_request
    while self._needs_retry(
../.venv/lib/python3.14/site-packages/botocore/endpoint.py:360: in _needs_retry
    responses = self._event_emitter.emit(
../.venv/lib/python3.14/site-packages/botocore/hooks.py:412: in emit
    return self._emitter.emit(aliased_event_name, **kwargs)
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/botocore/hooks.py:256: in emit
    return self._emit(event_name, kwargs)
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/botocore/hooks.py:239: in _emit
    response = handler(**kwargs)
               ^^^^^^^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/botocore/retryhandler.py:207: in __call__
    if self._checker(**checker_kwargs):
       ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/botocore/retryhandler.py:284: in __call__
    should_retry = self._should_retry(
../.venv/lib/python3.14/site-packages/botocore/retryhandler.py:320: in _should_retry
    return self._checker(attempt_number, response, caught_exception)
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/botocore/retryhandler.py:363: in __call__
    checker_response = checker(
../.venv/lib/python3.14/site-packages/botocore/retryhandler.py:247: in __call__
    return self._check_caught_exception(
../.venv/lib/python3.14/site-packages/botocore/retryhandler.py:416: in _check_caught_exception
    raise caught_exception
../.venv/lib/python3.14/site-packages/botocore/endpoint.py:279: in _do_get_response
    http_response = self._send(request)
                    ^^^^^^^^^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/botocore/endpoint.py:383: in _send
    return self.http_session.send(request)
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
../.venv/lib/python3.14/site-packages/botocore/httpsession.py:516: in send
    raise ConnectionClosedError(
E   botocore.exceptions.ConnectionClosedError: Connection was closed before we received a valid response from endpoint URL: "https://us-east-1.s3.fil.one:443/fth-compat-9yoh5l9c0v5c6mubzg-481/testobj".
```
...
**Error stack trace** — `encryption` — `failed` — 315.287s
...
../.venv/lib/python3.14/site-packages/botocore/httpsession.py:516: in send
    raise ConnectionClosedError(
E   botocore.exceptions.ConnectionClosedError: Connection was closed before we received a valid response from endpoint URL: "https://us-east-1.s3.fil.one:443/fth-compat-9yoh5l9c0v5c6mubzg-482/testobj".
```
...
**Error stack trace** — `encryption` — `failed` — 5.849s
...
**Error stack trace** — `checksum` — `failed` — 52.503s
...
**Error stack trace** — `bucket_logging` — `failed` — 7.206s
...
**Error stack trace** — `bucket_logging` — `failed` — 8.519s
...
**Error stack trace** — `bucket_logging` — `failed` — 7.416s
...
**Error stack trace** — `bucket_logging` — `failed` — 6.567s
...
**Error stack trace** — `bucket_logging` — `failed` — 8.068s
...
**Error stack trace** — `bucket_logging` — `failed` — 6.641s
...
**Error stack trace** — `s3_core` — `failed` — 5.455s
...
**Error stack trace** — `conditional_write` — `failed` — 6.364s
...
**Error stack trace** — `conditional_write` — `failed` — 10.993s
...
**Error stack trace** — `conditional_write` — `failed` — 8.145s
...
**Error stack trace** — `conditional_write` — `failed` — 6.686s
...
**Error stack trace** — `s3_core` — `failed` — 13.466s
...
**Error stack trace** — `s3_core` — `failed` — 12.964s
...
**Error stack trace** — `s3_core` — `failed` — 14.779s
...
**Error stack trace** — `s3_core` — `failed` — 17.103s
...
**Error stack trace** — `s3_core` — `failed` — 17.609s
...
**Error stack trace** — `s3_core` — `failed` — 11.855s
...
**Error stack trace** — `s3_core` — `failed` — 5.864s
...
E    +  where 404 = _get_status({'Error': {'Code': '404', 'Message': 'Not Found'}, 'ResponseMetadata': {'HTTPHeaders': {'content-length': '9', 'conten...ode': 404, 'HostId': '1783343517728464106-e721ca7e5924103d', 'RequestId': '1783343517728464106-e721ca7e5924103d', ...}})
E    +    where {'Error': {'Code': '404', 'Message': 'Not Found'}, 'ResponseMetadata': {'HTTPHeaders': {'content-length': '9', 'conten...ode': 404, 'HostId': '1783343517728464106-e721ca7e5924103d', 'RequestId': '1783343517728464106-e721ca7e5924103d', ...}} = ClientError('An error occurred (404) when calling the HeadObject operation: Not Found').response
```

</details>

<details markdown="1">
<summary>s3tests/functional/test_s3.py::test_copy_part_enc[sse-s3-sse-c-STANDARD-STANDARD-8388608]</summary>
...
**Error stack trace** — `encryption` — `failed` — 341.688s
...
s3tests/functional/test_s3.py:20594: in _test_copy_part_enc
    e = assert_raises(ClientError, client.upload_part,
s3tests/functional/utils.py:11: in assert_raises
...
../.venv/lib/python3.14/site-packages/botocore/httpsession.py:516: in send
    raise ConnectionClosedError(
E   botocore.exceptions.ConnectionClosedError: Connection was closed before we received a valid response from endpoint URL: "https://us-east-1.s3.fil.one:443/fth-compat-9yoh5l9c0v5c6mubzg-551/testobj2?partNumber=2&uploadId=2~mx7u-bkk3Qhi4BewYqyTFZmmO5t9KFD".
```
...
**Error stack trace** — `encryption` — `failed` — 31.639s
...
<details markdown="1">
<summary>s3tests/functional/test_s3.py::test_copy_part_enc[sse-c-sse-c-STANDARD-STANDARD-8388608]</summary>

**Test description**

> Parametrized UploadPartCopy matrix across (source_mode_key, dest_mode_key, source_storage_class, dest_storage_class, obj_size); verifies that encryption-mode transitions during copy preserve bytes.

**Error stack trace** — `encryption` — `failed` — 338.923s
...
../.venv/lib/python3.14/site-packages/botocore/httpsession.py:516: in send
    raise ConnectionClosedError(
E   botocore.exceptions.ConnectionClosedError: Connection was closed before we received a valid response from endpoint URL: "https://us-east-1.s3.fil.one:443/fth-compat-9yoh5l9c0v5c6mubzg-557/testobj2?partNumber=2&uploadId=2~BPwVCW0T9_TEYN8buE6_nMR9QNQSOrj".
```
...
**Error stack trace** — `encryption` — `failed` — 29.352s
...
**Error stack trace** — `encryption` — `failed` — 56.592s
...
**Error stack trace** — `encryption` — `failed` — 22.141s
...
**Error stack trace** — `encryption` — `failed` — 60.906s
...
**Error stack trace** — `encryption` — `failed` — 8.039s
...
**Error stack trace** — `encryption` — `failed` — 7.173s
...
**Error stack trace** — `encryption` — `failed` — 9.341s
...
**Error stack trace** — `encryption` — `failed` — 27.769s
...
**Error stack trace** — `encryption` — `failed` — 7.272s
...
**Error stack trace** — `encryption` — `failed` — 7.762s
...
**Error stack trace** — `encryption` — `failed` — 9.427s
...
**Error stack trace** — `encryption` — `failed` — 28.59s
...
**Error stack trace** — `encryption` — `failed` — 5.56s
...
**Error stack trace** — `encryption` — `failed` — 5.777s
...
**Error stack trace** — `encryption` — `failed` — 310.307s
...
../.venv/lib/python3.14/site-packages/botocore/httpsession.py:516: in send
    raise ConnectionClosedError(
E   botocore.exceptions.ConnectionClosedError: Connection was closed before we received a valid response from endpoint URL: "https://us-east-1.s3.fil.one:443/fth-compat-9yoh5l9c0v5c6mubzg-613/testobj".
```
...
**Error stack trace** — `encryption` — `failed` — 110.789s
...
**Error stack trace** — `encryption` — `failed` — 6.019s
...
**Error stack trace** — `encryption` — `failed` — 5.721s
...
**Error stack trace** — `encryption` — `failed` — 177.344s
...
**Error stack trace** — `encryption` — `failed` — 112.15s
...
**Error stack trace** — `encryption` — `failed` — 5.69s
...
**Error stack trace** — `encryption` — `failed` — 5.515s
...
**Error stack trace** — `encryption` — `failed` — 186.872s
...
**Error stack trace** — `encryption` — `failed` — 312.125s
...
../.venv/lib/python3.14/site-packages/botocore/httpsession.py:516: in send
    raise ConnectionClosedError(
E   botocore.exceptions.ConnectionClosedError: Connection was closed before we received a valid response from endpoint URL: "https://us-east-1.s3.fil.one:443/fth-compat-9yoh5l9c0v5c6mubzg-622/testobj".
```
